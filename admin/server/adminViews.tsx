...
import { renderPageById } from "site/server/SiteBaker"
...
const makeAdminRouter = (
    ...
    adminViews.post("/register", async (req, res) => {
        try {
            ...
            await tryLogin(req.body.email, req.body.password, serverSettings)
            res.redirect("/admin")
        } catch (err) {
            res.status(tryInt(err.code, 500))
            res.send(
    ...
    adminViews.get("/posts/preview/:postId", async (req, res) => {
        ...
        res.send(
            await renderPageById(postId, clientSettings, serverSettings, true)
        )
    })
